...
selected_year = st.sidebar.selectbox("เลือกปี", years, index=len(years) - 1)
ydf = prov[prov["year"] == selected_year].copy()

valid_months = ydf[["month_no", "month"]].drop_duplicates().sort_values("month_no")
month_options = [f"{r.month} ({int(r.month_no)})" for r in valid_months.itertuples() if int(r.month_no) > 0]
...
view = ydf[ydf["month_no"] == selected_month].copy()
...
capital_df = load_province_capital_category(selected_year)
size_df = load_province_size(selected_year)
...
with tab4:
    st.markdown(f"## คำถาม: ใน **{selected_province}** เงินทุนไหลไปกลุ่มธุรกิจใดมากที่สุด?")
    st.caption(data_caption)
    if capital_df.empty:
        st.warning("ยังไม่พบ/อ่านไฟล์จังหวัดมูลค่าทุน*_cat.csv หรือไฟล์ต้นทางที่เกี่ยวข้อง")
        with st.expander("Debug: ชื่อไฟล์ที่ระบบเห็น"):
            st.write([f.name for f in _all_data_files()])
            st.write("size candidates:", [f.name for f in _file_candidates("size", selected_year)] if "_file_candidates" in globals() else [])
            st.write("capital candidates:", [f.name for f in _file_candidates("capital", selected_year)] if "_file_candidates" in globals() else [])
    else:
        selected_capital = capital_df[capital_df["province"] == selected_province].copy()
        selected_capital = selected_capital.groupby("group", as_index=False)["capital_m"].sum().sort_values("capital_m", ascending=False)
        if selected_capital.empty:
            st.warning(f"ยังไม่มีข้อมูลมูลค่าทุนของ {selected_province} ในไฟล์ที่อ่านได้")
        elif selected_capital["group"].nunique() <= 1:
            only_group = selected_capital.iloc[0]
            st.info("แท็บนี้ซ่อนกราฟไว้ชั่วคราว เพราะข้อมูลจังหวัดนี้มีเพียง 1 กลุ่มธุรกิจ จึงยังไม่ช่วยให้ผู้ใช้เห็นภาพเปรียบเทียบเพิ่มเติม")
            st.markdown("<div class='glass-card'><div class='section-eyebrow'>Capital Snapshot</div>", unsafe_allow_html=True)
            st.markdown(f"### จังหวัด {selected_province}")
            st.markdown(f"- กลุ่มธุรกิจที่พบในไฟล์ทุน: **{only_group['group']}**")
            st.markdown(f"- มูลค่าทุนที่อ่านได้: **{_fmt_float(only_group['capital_m'])} ล้านบาท**")
            st.markdown("- เมื่อมีข้อมูลหลายกลุ่มมากขึ้น ระบบจะแสดงกราฟเปรียบเทียบให้อัตโนมัติ")
            st.markdown("</div>", unsafe_allow_html=True)
        else:
            col1, col2 = st.columns([1, 1])
            with col1:
                st.plotly_chart(capital_pie(capital_df, selected_province), use_container_width=True)
            with col2:
                group_rank = selected_capital.copy()
                top_group = group_rank.iloc[0]
                total_cap = group_rank["capital_m"].sum()
                share = (top_group.capital_m / total_cap * 100) if total_cap else 0
                st.markdown("<div class='glass-card'><div class='section-eyebrow'>Market Insight</div>", unsafe_allow_html=True)
                st.markdown(f"### กลุ่มเด่น: {top_group.group}")
                st.markdown(f"- มูลค่าทุนรวมสูงสุด: **{_fmt_float(top_group.capital_m)} ล้านบาท**")
                st.markdown(f"- คิดเป็นประมาณ **{_fmt_float(share)}%** ของมูลค่าทุนรวมที่อ่านได้")
                st.markdown("- ใช้ตอบว่าเงินทุนกำลังไหลไปที่ตลาดไหนมากที่สุด")
                st.markdown("</div>", unsafe_allow_html=True)
                st.dataframe(group_rank.rename(columns={"group": "กลุ่มธุรกิจ", "capital_m": "มูลค่าทุน (ลบ.)"}), use_container_width=True, hide_index=True)
