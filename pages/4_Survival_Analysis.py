...
def monthly_bubble(df: pd.DataFrame, selected_types: list[str]) -> go.Figure:
    ...
    for _, r in d.iterrows():
        for m in month_cols:
            rows.append({
                ...
                "quit_count": r.get(m, 0),
                "risk_score": r.get("risk_score", 0),
            })
    long = pd.DataFrame(rows)
    long = long[long["quit_count"] >= 0]
    if long.empty:
        return go.Figure()
    long["short_type"] = long["type"].astype(str).apply(lambda x: x if len(x) < 38 else x[:38] + "…")
    fig = px.scatter(
        long,
        x="month",
        y="short_type",
    ...
    fig.update_traces(hovertemplate="%{customdata[0]}<br>กลุ่ม: %{customdata[1]}<br>เดือน: %{x}<br>เลิกกิจการ: %{customdata[2]:,.0f} ราย<br>Risk Score: %{customdata[3]:.1f}<extra></extra>")
    fig.update_layout(xaxis_title="เดือน ปี 2569", yaxis_title="ประเภทธุรกิจ", coloraxis_colorbar_title="เลิกกิจการ")
    return fig_layout(fig, "Bubble Plot: เดือนที่มีความเสี่ยงจากการเลิกกิจการ", height=max(540, min(900, len(long["type"].unique()) * 42 + 200)), showlegend=False)
...
with tab1:
    ...
    st.caption(f"ข้อมูล: ประเภทจัดตั้งเลิกขนาด69_active + ประเภทจัดตั้งเลิกขนาด69_quit · เดือนล่าสุด {LATEST_MONTH_TEXT}")
    ...
    with f4:
        mode_1 = st.selectbox("มุมมองกราฟ", ["เสี่ยงสูง", "รอดสูง"], key="surv_mode")
    ...
    if valid1.empty:
        ...
        st.plotly_chart(survival_compare_bar(base1, threshold_1, int(n_show), mode_1), use_container_width=True, key="survival_compare_bar_tab1")
        st.info("หมายเหตุ: Survival Rate มักสูงมาก เพราะตัวหารใช้ฐานธุรกิจคงอยู่ที่มีจำนวนมาก เทียบกับจำนวนเลิกในช่วง ม.ค.–เม.ย. 2569 ดังนั้นกราฟจึงเน้น ‘อัตราร่วง = 100 - Survival Rate’ เพื่อให้เห็นความแตกต่างได้ชัดกว่า")
        low = valid1.sort_values("survival_rate", ascending=True).head(5)
        high = valid1.sort_values("survival_rate", ascending=False).head(5)
        c1, c2 = st.columns(2)
        with c1:
            st.markdown("<div class='glass-card'><div class='section-eyebrow'>ควรระวัง / ต่ำกว่าเพื่อนในกลุ่ม</div>", unsafe_allow_html=True)
            for r in low.itertuples():
                st.markdown(f"- **{r.type}** — Survival {_fmt_float(r.survival_rate)}% | เลิก {_fmt_int(r.total_quit)} ราย | คงอยู่ {_fmt_int(r.active_count)} ราย")
            st.markdown("</div>", unsafe_allow_html=True)
        with c2:
            st.markdown("<div class='glass-card'><div class='section-eyebrow'>อยู่รอดสูง</div>", unsafe_allow_html=True)
            for r in high.itertuples():
                note = " (ไม่พบการเลิกในช่วงนี้ จึงได้ 100%)" if r.total_quit == 0 else ""
                st.markdown(f"- **{r.type}** — Survival {_fmt_float(r.survival_rate)}% | คงอยู่ {_fmt_int(r.active_count)} ราย{note}")
            st.markdown("</div>", unsafe_allow_html=True)
        if biz68.empty:
            st.info("หมายเหตุ: ระบบยังไม่พบไฟล์ปี 2568 ที่มีทั้ง active และ quit ครบ จึงแสดงเฉพาะปี 2569 แต่ไม่ได้แต่งข้อมูลเพิ่ม")
        else:
            st.info("การเทียบปี 2568 ช่วยเพิ่มบริบทว่าอัตราอยู่รอดปีล่าสุดสูง/ต่ำกว่าปีก่อนหรือไม่ ส่วนค่า Survival สูงมากเกิดจากฐานธุรกิจคงอยู่มีจำนวนมากเมื่อเทียบกับจำนวนเลิกในช่วง ม.ค.–เม.ย. ไม่ใช่การสุ่มข้อมูล")
...
with tab5:
    ...
    if not selected_bubble:
        ...
        if not rows or max([x[2] for x in rows]) == 0:
            st.markdown("ธุรกิจที่เลือกไม่มีจำนวนเลิกกิจการรายเดือนในช่วง ม.ค.–เม.ย. 2569 จึงไม่ควรฝืนสรุปเดือนเสี่ยง")
        else:
