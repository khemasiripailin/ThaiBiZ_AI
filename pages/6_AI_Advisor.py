...
def call_gemini(prompt, model_name, api_key=None):
    ...
    try:
        ...
        try:
            ...
            return None, f"เรียก Gemini ไม่สำเร็จ: {e_new} / {e_old}"
...
st.markdown("## 2) Suggested Questions")
suggested = [
...
btn_rows = [st.columns(3), st.columns(3)]
for i, q in enumerate(suggested):
    with btn_rows[i // 3][i % 3]:
        if st.button(q, key=f"suggested_{i}"):
            st.session_state["advisor_question"] = q
...
with st.expander("ตั้งค่า Gemini API", expanded=False):
    ...
    if current_has_key:
        ...
        st.info("ถ้าเพิ่งสร้าง .env ให้หยุดรัน Streamlit แล้วเปิดใหม่อีกครั้ง")

question = st.chat_input("ถาม เช่น ควรเริ่มธุรกิจอะไรในงบ 1 ล้านบาท?")
if question:
    st.session_state["advisor_question"] = question

if st.session_state["advisor_question"]:
    user_q = st.session_state["advisor_question"]
    ...
    with st.chat_message("assistant"):
        if use_ai:
            ...
            if ai_text:
                answer_card(ai_text)
                st.caption("ตอบโดย Gemini โดยจำกัด context จากข้อมูลใน dashboard นี้")
            else:
                st.warning(f"ใช้ Gemini ไม่ได้ จึงแสดงคำตอบแบบ rule-based จากข้อมูลจริงแทน: {err}")
                answer_card(fallback_answer(goal, budget, risk, group, area, user_q))
        else:
            answer_card(fallback_answer(goal, budget, risk, group, area, user_q))
            st.caption("ตอบแบบ rule-based จากข้อมูลจริงใน dashboard; เปิด Gemini API ได้ใน expander ด้านบน")
