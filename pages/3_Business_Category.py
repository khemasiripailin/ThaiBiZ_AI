...
def _matcher_reason(row: pd.Series, budget: float, threshold: float) -> list[str]:
    reasons = []
    if float(row.get("avg_capital_m", 0)) <= budget * 0.60:
        reasons.append("ใช้ทุนต่ำกว่างบค่อนข้างมาก")
    elif float(row.get("avg_capital_m", 0)) <= budget:
        reasons.append("ใช้ทุนอยู่ในกรอบงบที่ตั้งไว้")

    if float(row.get("closure_rate", 0)) <= threshold * 0.50:
        reasons.append("ความเสี่ยงต่ำกว่าเกณฑ์ที่ยอมรับได้มาก")
    elif float(row.get("closure_rate", 0)) <= threshold:
        reasons.append("ความเสี่ยงยังอยู่ในระดับที่รับได้")

    if float(row.get("regis_count", 0)) >= 500:
        reasons.append("มีสัญญาณ demand สูงจากจำนวนจัดตั้งใหม่มาก")
    elif float(row.get("regis_count", 0)) >= 150:
        reasons.append("มีจำนวนจัดตั้งใหม่ระดับกลางถึงสูง")
    else:
        reasons.append("เป็นตลาดเฉพาะที่จำนวนผู้เล่นใหม่ยังไม่สูงมาก")
    return reasons[:3]


def matcher_rank_chart(df: pd.DataFrame, highlight_type: str | None = None) -> go.Figure:
    d = df.copy().head(10)
    if d.empty:
        return go.Figure()
    d = d.sort_values("opportunity_score", ascending=True).copy()
    d["type_wrapped"] = d["type"].apply(lambda s: _wrap_label(s, width=34, max_lines=3))
    d["is_selected"] = d["type"].eq(highlight_type) if highlight_type else False

    category_order = d["type_wrapped"].tolist()
    normal = d[~d["is_selected"]].copy()
    selected = d[d["is_selected"]].copy()

    fig = go.Figure()
    if not normal.empty:
        fig.add_trace(go.Bar(
            x=normal["opportunity_score"],
            y=normal["type_wrapped"],
            orientation="h",
            marker=dict(color=[GROUP_COLORS.get(g, PRIMARY) for g in normal["business_group"]], opacity=0.55),
            text=normal["opportunity_score"].map(lambda x: f"{x:,.1f}"),
            textposition="outside",
            cliponaxis=False,
            customdata=np.stack([normal["type"], normal["business_group"], normal["avg_capital_m"], normal["regis_count"], normal["closure_rate"]], axis=1),
            hovertemplate=(
                "<b>%{customdata[0]}</b><br>กลุ่ม: %{customdata[1]}<br>Opportunity Score: %{x:.1f}"
                "<br>ทุนเฉลี่ย: %{customdata[2]:,.2f} ลบ./ราย<br>จัดตั้งใหม่: %{customdata[3]:,.0f} ราย"
                "<br>Closure Rate: %{customdata[4]:.2f}%<extra></extra>"
            ),
            name="ธุรกิจใน shortlist",
            showlegend=False,
        ))

    if not selected.empty:
        fig.add_trace(go.Bar(
            x=selected["opportunity_score"],
            y=selected["type_wrapped"],
            orientation="h",
            marker=dict(color="#f59e0b", line=dict(color="#fde68a", width=1.8), opacity=0.98),
            text=selected["opportunity_score"].map(lambda x: f"{x:,.1f}  ★"),
            textposition="outside",
            cliponaxis=False,
            customdata=np.stack([selected["type"], selected["business_group"], selected["avg_capital_m"], selected["regis_count"], selected["closure_rate"]], axis=1),
            hovertemplate=(
                "<b>%{customdata[0]}</b><br>กลุ่ม: %{customdata[1]}<br>Opportunity Score: %{x:.1f}"
                "<br>ทุนเฉลี่ย: %{customdata[2]:,.2f} ลบ./ราย<br>จัดตั้งใหม่: %{customdata[3]:,.0f} ราย"
                "<br>Closure Rate: %{customdata[4]:.2f}%<extra></extra>"
    ...
    fig.update_layout(
        xaxis_title="Opportunity Score (0-100)",
        yaxis_title="ธุรกิจที่แนะนำ",
        showlegend=False,
        margin=dict(t=95, l=20, r=50, b=55),
        xaxis=dict(range=[0, max(100, float(d["opportunity_score"].max()) + 8)]),
        yaxis=dict(automargin=True, tickfont=dict(size=11), categoryorder="array", categoryarray=category_order),
    )
    title = "Top ธุรกิจที่เหมาะกับงบและความเสี่ยง"
    if highlight_type:
        title += " · ไฮไลต์ตัวเลือกที่สนใจ"
    return fig_layout(fig, title=title, height=max(520, len(d) * 52 + 170), showlegend=False)


def matcher_score_badge(score: float) -> str:
    if score >= 75:
        return "เหมาะมาก"
    if score >= 60:
        return "น่าพิจารณา"
...
def render_matcher_card(row: pd.Series, rank: int, budget: float, threshold: float):
    reasons = _matcher_reason(row, budget, threshold)
    badge = matcher_score_badge(float(row.get("opportunity_score", 0)))
    reason_html = "<br>• ".join(reasons)
    st.markdown(
        f"""
        <div class='glass-card' style='height: 100%;'>
            <div class='section-eyebrow'>Top Pick #{rank}</div>
            <h4 style='margin:.15rem 0 .5rem 0; color:#f8fafc;'>{row['type']}</h4>
            <div style='font-size:.95rem; color:{WHITE_SOFT}; margin-bottom:.6rem;'>กลุ่มธุรกิจ: {row['business_group']}</div>
            <div style='display:flex; gap:.45rem; flex-wrap:wrap; margin-bottom:.7rem;'>
                <span style='padding:.22rem .55rem; border-radius:999px; background:rgba(168,85,247,.16); border:1px solid rgba(168,85,247,.35); color:#e9d5ff; font-size:.82rem;'>Score {row['opportunity_score']:.1f}</span>
                <span style='padding:.22rem .55rem; border-radius:999px; background:rgba(34,197,94,.14); border:1px solid rgba(34,197,94,.32); color:#bbf7d0; font-size:.82rem;'>{badge}</span>
            </div>
            <ul class='insight-list' style='margin:0;'>
                <li>ทุนเฉลี่ยต่อราย: <b>{_fmt_float(row['avg_capital_m'])}</b> ล้านบาท</li>
                <li>จัดตั้งใหม่: <b>{_fmt_int(row['regis_count'])}</b> ราย</li>
                <li>Closure Rate: <b>{_fmt_float(row['closure_rate'])}%</b></li>
            </ul>
            <div style='margin-top:.7rem; color:#cbd5e1; font-size:.92rem;'><b>เหตุผลที่แนะนำ</b><br>• {reason_html}</div>
        </div>
        """,
...
def matcher_insight_text(df: pd.DataFrame) -> str:
    if df.empty:
        return ""
    best = df.sort_values(["opportunity_score", "regis_count"], ascending=[False, False]).iloc[0]
    safest = df.sort_values("closure_rate", ascending=True).iloc[0]
    cheapest = df.sort_values("avg_capital_m", ascending=True).iloc[0]
    return (
        f"ตัวเลือกเด่นสุดในรอบนี้คือ <b>{best['type']}</b> (Score {best['opportunity_score']:.1f}) · "
        f"ตัวเลือกที่เสี่ยงต่ำสุดคือ <b>{safest['type']}</b> (Closure Rate {safest['closure_rate']:.2f}%) · "
        f"ตัวเลือกที่ใช้ทุนต่ำสุดคือ <b>{cheapest['type']}</b> ({_fmt_float(cheapest['avg_capital_m'])} ลบ./ราย)"
    )


def render_kpi_card_html(title: str, value: str, subtitle: str = "", accent: str = PRIMARY):
    st.markdown(
        f"""
        <div style="padding:1rem 1.05rem; border-radius:18px; min-height:118px;
                    background:linear-gradient(135deg, rgba(15,23,42,.96), rgba(30,41,59,.78));
                    border:1px solid {accent}55; box-shadow:0 14px 32px rgba(0,0,0,.22);">
            <div style="font-size:.78rem; letter-spacing:.08em; text-transform:uppercase; font-weight:700; color:{accent};">{title}</div>
            <div style="font-size:1.75rem; font-weight:800; color:#f8fafc; margin:.38rem 0 .28rem 0; line-height:1.15;">{value}</div>
            <div style="font-size:.92rem; color:{WHITE_SOFT};">{subtitle}</div>
        </div>
        """,
        unsafe_allow_html=True,
    )
...
def render_html_panel(title: str, body_html: str, accent: str = ACCENT):
    st.markdown(
        f"""
        <div style="padding:1rem 1.1rem; border-radius:18px; height:100%;
                    background:linear-gradient(135deg, rgba(15,23,42,.94), rgba(15,23,42,.82));
                    border:1px solid rgba(148,163,184,.22); box-shadow:0 10px 24px rgba(0,0,0,.16);">
            <div style="font-size:.82rem; letter-spacing:.06em; text-transform:uppercase; font-weight:700; color:{accent}; margin-bottom:.55rem;">{title}</div>
            <div style="color:#e2e8f0; font-size:.95rem; line-height:1.68;">{body_html}</div>
        </div>
        """,
        unsafe_allow_html=True,
    )
...
def capital_box_dynamic_summary(box_data: pd.DataFrame, trim_data: pd.DataFrame, selected_group: str):
    """สรุป Box Plot ให้ดูเป็น dashboard และอิงตามตัวกรองที่เลือก"""
    source = trim_data.copy() if trim_data is not None and not trim_data.empty else box_data.copy()
    if source is None or source.empty:
        return

    source["avg_capital_m"] = pd.to_numeric(source["avg_capital_m"], errors="coerce")
    source = source[source["avg_capital_m"].notna() & np.isfinite(source["avg_capital_m"])]
    if source.empty:
        return

    focus = source.copy() if selected_group == "ทั้งหมด" else source[source["business_group"] == selected_group].copy()
    if focus.empty:
        st.info("ยังไม่มีข้อมูลพอสำหรับสรุปผลตามตัวกรองนี้")
        return

    overall = source.copy()
    med = float(focus["avg_capital_m"].median())
    q1 = float(focus["avg_capital_m"].quantile(.25))
    q3 = float(focus["avg_capital_m"].quantile(.75))
    overall_med = float(overall["avg_capital_m"].median())
    direction = "สูงกว่า" if med > overall_med else "ต่ำกว่า" if med < overall_med else "ใกล้เคียงกับ"
    count_types = int(len(focus))
    top_rows = focus.sort_values("avg_capital_m", ascending=False).head(3)

    st.markdown("#### สรุปตามตัวกรองที่เลือก")
    k1, k2, k3 = st.columns(3)
    with k1:
        render_kpi_card_html("ประเภทธุรกิจในกราฟ", _fmt_int(count_types), f"{selected_group if selected_group != 'ทั้งหมด' else 'ภาพรวมทุกกลุ่ม'}", PRIMARY)
    with k2:
        render_kpi_card_html("ค่ากลางทุนเฉลี่ย", f"{_fmt_float(med)} ลบ./ราย", f"{direction}ค่ากลางภาพรวม {_fmt_float(overall_med)} ลบ./ราย", ACCENT)
    with k3:
        render_kpi_card_html("ช่วง 50% กลาง", f"{_fmt_float(q1)} – {_fmt_float(q3)}", "ช่วงที่ธุรกิจส่วนใหญ่กระจุกตัว", YELLOW)

    left, right = st.columns([1.15, 1])
    with left:
        if not top_rows.empty:
            items = "".join([
                f"<li><b>{r.type}</b> — {_fmt_float(r.avg_capital_m)} ล้านบาท/ราย</li>" for r in top_rows.itertuples()
            ])
        else:
            items = "<li>ยังไม่มีรายการเด่นพอสำหรับแสดง</li>"
        render_html_panel(
            "ธุรกิจที่ใช้ทุนสูงในช่วงที่กำลังดู",
            f"<ul class='insight-list'>{items}</ul>",
            accent=PINK,
        )
    with right:
        group_text = selected_group if selected_group != "ทั้งหมด" else "ภาพรวมทุกกลุ่ม"
        body = f"""
        <ul class='insight-list'>
            <li><b>กลุ่มที่กำลังอ่าน:</b> {group_text}</li>
            <li>ค่ากลางทุนเฉลี่ยต่อรายอยู่ที่ <b>{_fmt_float(med)} ล้านบาท</b> และ 50% ของข้อมูลอยู่ในช่วง <b>{_fmt_float(q1)} – {_fmt_float(q3)} ล้านบาท</b></li>
            <li>ถ้าช่วง 50% กลางยังสูงกว่างบของเรา ให้ใช้หน้า <b>Business Matcher</b> เพื่อ shortlist ธุรกิจที่เข้ากับงบประมาณ</li>
            <li>ถ้าธุรกิจที่ทุนสูงสุดดูน่าสนใจ ให้ไปดูต่อที่ <b>Scatter Plot</b> เพื่อเช็กว่าคนเปิดใหม่เยอะแค่ไหนและตลาดแข่งขันหนักหรือไม่</li>
        </ul>
        """
        render_html_panel("กราฟนี้สื่ออะไรและควรทำอะไรต่อ", body, accent=GREEN)


def render_scatter_group_summary(df: pd.DataFrame, selected_group: str):
    d = df[(pd.to_numeric(df["avg_capital_m"], errors="coerce") > 0) & (pd.to_numeric(df["regis_count"], errors="coerce") > 0)].copy()
    if d.empty:
        return
    focus = d.copy() if selected_group == "ทั้งหมด" else d[d["business_group"] == selected_group].copy()
    if focus.empty:
        return

    med_regis = float(focus["regis_count"].median())
    med_cap = float(focus["avg_capital_m"].median())
    avg_close = float(focus["closure_rate"].mean())
    top_regis = focus.sort_values(["regis_count", "opportunity_score"], ascending=[False, False]).iloc[0]
    niche = focus.sort_values(["avg_capital_m", "closure_rate", "regis_count"], ascending=[True, True, False]).iloc[0]

    st.markdown("#### สรุปตามกลุ่มธุรกิจที่เลือก")
    s1, s2, s3 = st.columns(3)
    with s1:
        render_kpi_card_html("ธุรกิจในมุมมองนี้", _fmt_int(len(focus)), f"{selected_group if selected_group != 'ทั้งหมด' else 'ภาพรวมทุกกลุ่ม'}", PRIMARY)
    with s2:
        render_kpi_card_html("ค่ากลางจำนวนเปิดใหม่", f"{_fmt_int(med_regis)} ราย", "Median ของ regis_count", ACCENT)
    with s3:
        render_kpi_card_html("ค่ากลางทุนเฉลี่ย", f"{_fmt_float(med_cap)} ลบ./ราย", f"Closure Rate เฉลี่ย {_fmt_float(avg_close)}%", PINK)

    a, b = st.columns(2)
    with a:
        body = f"""
        <ul class='insight-list'>
            <li><b>ธุรกิจที่เปิดใหม่มากสุดในมุมมองนี้:</b> {top_regis['type']} ({_fmt_int(top_regis['regis_count'])} ราย)</li>
            <li><b>ค่ากลางของกลุ่มนี้</b> อยู่ที่จำนวนจัดตั้งใหม่ประมาณ {_fmt_int(med_regis)} ราย และทุนเฉลี่ยต่อราย {_fmt_float(med_cap)} ล้านบาท</li>
            <li>ถ้าจุดกระจุกทางขวา แปลว่ามี demand สูงขึ้น แต่ควรเช็ก Closure Rate เพิ่มเพื่อไม่ให้เข้าไปในตลาดที่แข่งหนักเกินไป</li>
        </ul>
        """
        render_html_panel("สิ่งที่เห็นจากกราฟนี้", body, accent=ACCENT)
    with b:
        body = f"""
        <ul class='insight-list'>
            <li><b>จุดเริ่มต้นที่ใช้ทุนน้อยกว่า:</b> {niche['type']} (ทุนเฉลี่ย {_fmt_float(niche['avg_capital_m'])} ลบ./ราย)</li>
            <li>ถ้าอยากหาธุรกิจที่เข้าได้ง่ายขึ้น ให้เริ่มจากจุดที่ <b>ทุนไม่สูงเกิน</b> และ <b>จำนวนเปิดใหม่ยังมีอยู่</b> เพื่อไม่ให้เป็นตลาดที่เงียบเกินไป</li>
            <li><b>สิ่งที่ควรทำต่อ</b>: ใช้หน้า <b>Business Matcher</b> ใส่งบประมาณของตนเอง แล้วดู shortlist ที่เข้ากับความเสี่ยงที่รับได้</li>
        </ul>
        """
        render_html_panel("แปลผลเป็น action ได้อย่างไร", body, accent=GREEN)
...
st.sidebar.caption("ใช้กับแท็บ 1) Market Demand · 2) Opportunity Map · 3) Opportunity Score · 4) SME Fit · 5) Capital Readiness · 6) Business Matcher · 7) Micro-Niche · 8) Raw Data")
...
st.sidebar.caption("ใช้กับแท็บ 6) Business Matcher")
...
# ============================================================
with tab5:
    st.markdown("## คำถาม: ต้องใช้เงินทุนประมาณเท่าไร และธุรกิจกลุ่มไหนเข้ากับงบของเรา?")
    st.caption("ข้อมูลจาก DBD ช่วงมกราคม 2568 ถึง เมษายน 2569 · ใช้ดู distribution ของทุนเฉลี่ยต่อราย, outliers, ขนาดธุรกิจ และความพร้อมด้านเงินทุน")

    # 1) Full box plot
    st.markdown("### 1) Box Plot: ทุนเฉลี่ยต่อราย")
    full_fig, box_data = capital_box(base_view, selected_group, remove_outliers=False)
    if box_data.empty:
        st.info("ไม่มีข้อมูลทุนเฉลี่ยต่อรายพอสำหรับสร้าง Box Plot")
    else:
        st.plotly_chart(full_fig, use_container_width=True)
        st.caption("กราฟนี้ใช้ดูการกระจายของทุนเฉลี่ยต่อรายทั้งหมด เพื่อเห็นค่ากลาง ช่วงกระจุกตัว และธุรกิจที่อาจใช้ทุนสูงมากผิดจากกลุ่มหลัก")

    st.markdown("---")
    # 2) Trimmed box plot + dashboard summary
    st.markdown("### 2) Box Plot แบบตัด Outliers")
    trim_fig, trim_data = capital_box(base_view, selected_group, remove_outliers=True)
    if trim_data.empty:
        st.info("ไม่มีข้อมูลหลังตัด outliers")
    else:
        st.plotly_chart(trim_fig, use_container_width=True)
        st.caption("เมื่อตัด outliers ออก จะช่วยให้เห็นภาพรวมของระดับทุนที่ธุรกิจส่วนใหญ่ใช้จริงได้ชัดขึ้น และเหมาะกับการเทียบกับงบของผู้ใช้")
        capital_box_dynamic_summary(box_data, trim_data, selected_group)

    st.markdown("---")
    # 3) Scatter + dynamic summary
    st.markdown("### 3) Scatter Plot: จำนวนจัดตั้งใหม่ vs ทุนเฉลี่ยต่อราย")
    sc = capital_scatter(base_view, selected_group)
    st.plotly_chart(sc, use_container_width=True)
    render_scatter_group_summary(base_view, selected_group)
    ...
    with c1:
        ...
        render_html_panel(
            "อธิบายกราฟ: จำนวนธุรกิจตามขนาด",
            "ถ้าแท่ง <b>S</b> สูงมาก แปลว่าตลาดมีผู้เล่นรายเล็กจำนวนมาก หากแท่ง <b>L</b> สูง แปลว่าตลาดมีผู้เล่นทุนใหญ่ค่อนข้างมาก เหมาะใช้ดูว่าตลาดนั้นเป็นสนามของ SME หรือของผู้เล่นทุนหนามากกว่า",
            accent=PRIMARY,
        )
    with c2:
        st.plotly_chart(size_avg_fig, use_container_width=True)
        render_html_panel(
            "อธิบายกราฟ: ทุนเฉลี่ยต่อราย",
            "ช่วยดูว่าขนาดธุรกิจใดต้องใช้ทุนต่อรายมากที่สุด เพื่อประเมินความพร้อมด้านการเงินก่อนเริ่มธุรกิจ และใช้ประกอบการตั้งงบในหน้า Business Matcher ต่อได้",
            accent=ACCENT,
        )
    ...
    if not size_summary.empty:
        ...
        with csum1:
            render_html_panel(
                "Size Summary",
                f"<ul class='insight-list'><li>ขนาดที่มีจำนวนธุรกิจมากสุดคือ <b>{top_size_count['size']}</b> ({_fmt_int(top_size_count['count'])} ราย)</li><li>ช่วงมูลค่าทุนที่ใช้ตีความในหน้าแอปคือ <b>S &lt; 1 ล้านบาท · M = 1–10 ล้านบาท · L &gt; 10 ล้านบาท</b></li></ul>",
                accent=PINK,
            )
        with csum2:
            render_html_panel(
                "สิ่งที่ผู้ใช้ควรทำต่อ",
                f"<ul class='insight-list'><li>ขนาดที่มีทุนเฉลี่ยต่อรายสูงสุดคือ <b>{top_size_avg['size']}</b> ({_fmt_float(top_size_avg['avg_capital_m'])} ล้านบาท/ราย)</li><li>ถ้าต้องการเริ่มแบบเสี่ยงน้อย ให้เริ่มสังเกตกลุ่มหรือขนาดที่ทุนเฉลี่ยยังสอดคล้องกับงบประมาณของตัวเอง</li></ul>",
                accent=GREEN,
            )
    ...
    if not view.empty:
        ...
        items = "".join([f"<li><b>{r.type}</b> — ทุนรวม {_fmt_float(r.capital_value)} ล้านบาท | ทุนเฉลี่ยต่อราย {_fmt_float(r.avg_capital_m)} ล้านบาท</li>" for r in capital_rank.itertuples()])
        render_html_panel("Treemap Summary", f"<ul class='insight-list'>{items}</ul>", accent=YELLOW)

with tab6:
    st.markdown("## คำถาม: ถ้ามีงบและรับความเสี่ยงได้ประมาณนี้ ควร shortlist ธุรกิจอะไร?")
    st.caption("Business Matcher คือกิมมิคหลักของหน้านี้ ใช้งบลงทุนโดยประมาณ + ระดับความเสี่ยงที่รับได้ เพื่อคัด shortlist ธุรกิจที่เหมาะกับผู้ใช้")
    ...
    if matcher.empty:
        st.warning("ยังไม่พบธุรกิจที่เข้าเงื่อนไขงบ/ความเสี่ยง ลองเพิ่มงบ หรือลดระดับความเข้มของตัวกรอง")
    else:
        highlight_options = matcher["type"].tolist()
        selected_type_focus = st.selectbox("เลือกธุรกิจที่ต้องการไฮไลต์ในกราฟ", highlight_options, index=0)
        focus_row = matcher[matcher["type"] == selected_type_focus].iloc[0]

        m1, m2, m3, m4 = st.columns(4)
        with m1:
            render_kpi_card_html("รายการที่ผ่านเงื่อนไข", _fmt_int(len(matcher)), f"งบ ≤ {_fmt_float(budget)} ลบ.", PRIMARY)
        with m2:
            render_kpi_card_html("Opportunity ของตัวที่เลือก", f"{focus_row['opportunity_score']:.1f}", matcher_score_badge(float(focus_row['opportunity_score'])), ACCENT)
        with m3:
            render_kpi_card_html("ทุนเฉลี่ยของตัวที่เลือก", f"{_fmt_float(focus_row['avg_capital_m'])} ลบ.", f"จัดตั้งใหม่ {_fmt_int(focus_row['regis_count'])} ราย", PINK)
        with m4:
            render_kpi_card_html("Closure Rate ของตัวที่เลือก", f"{_fmt_float(focus_row['closure_rate'])}%", f"เกณฑ์ที่รับได้ ≤ {_fmt_float(threshold,0)}%", YELLOW)

        st.plotly_chart(matcher_rank_chart(matcher, highlight_type=selected_type_focus), use_container_width=True)
        st.caption("แท่งสีเหลืองคือธุรกิจที่ผู้ใช้เลือกไฮไลต์ เพื่อให้เห็นตำแหน่งของธุรกิจนั้นเทียบกับ shortlist อื่นในงบและความเสี่ยงชุดเดียวกัน")

        info1, info2 = st.columns([1.2, 1])
        with info1:
            reasons = _matcher_reason(focus_row, budget, threshold)
            bullets = "".join([f"<li>{r}</li>" for r in reasons])
            render_html_panel(
                "ธุรกิจที่เลือก: วิเคราะห์แบบเจาะลึก",
                f"<div style='font-size:1.05rem; font-weight:800; color:#f8fafc; margin-bottom:.35rem;'>{focus_row['type']}</div>"
                f"<div style='margin-bottom:.55rem; color:{WHITE_SOFT};'>กลุ่มธุรกิจ: {focus_row['business_group']}</div>"
                f"<ul class='insight-list'><li><b>Opportunity Score</b>: {_fmt_float(focus_row['opportunity_score'],1)}</li><li><b>ทุนเฉลี่ยต่อราย</b>: {_fmt_float(focus_row['avg_capital_m'])} ล้านบาท</li><li><b>จัดตั้งใหม่</b>: {_fmt_int(focus_row['regis_count'])} ราย</li><li><b>Closure Rate</b>: {_fmt_float(focus_row['closure_rate'])}%</li></ul><div style='margin-top:.45rem; font-weight:700; color:#f8fafc;'>เหตุผลที่น่าสนใจ</div><ul class='insight-list'>{bullets}</ul>",
                accent=YELLOW,
            )
        with info2:
            shortlist = matcher.head(3)
            items = "".join([f"<li><b>{r.type}</b> — Score {_fmt_float(r.opportunity_score,1)} | ทุน {_fmt_float(r.avg_capital_m)} ลบ. | Closure {_fmt_float(r.closure_rate)}%</li>" for r in shortlist.itertuples()])
            render_html_panel(
                "Shortlist ที่ควรดูต่อ",
                f"<ul class='insight-list'>{items}</ul><div style='margin-top:.4rem;'><b>Next step</b>: เลือก 1–2 ธุรกิจจาก shortlist นี้ แล้วกลับไปดูหน้า <b>Opportunity Map</b> เพื่อเช็กตำแหน่งตลาด และหน้า <b>Micro-Niche Map</b> เพื่อมองจังหวัดที่น่าลงพื้นที่</div>",
                accent=GREEN,
            )

        with st.expander("ดูตารางเปรียบเทียบทั้งหมด", expanded=False):
            st.dataframe(
                _styled_matcher(matcher[["type", "business_group", "avg_capital_m", "regis_count", "closure_rate", "opportunity_score"]]),
