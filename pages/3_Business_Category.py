        ...
        if isinstance(rendered, pd.DataFrame) and not rendered.empty:
            ...
            with c2:
                ...
                for r in top_red.itertuples():
                    st.markdown(f"- **{r.province}** — Micro Score {_fmt_float(r.micro_score,1)} | จัดตั้ง {int(r.new_count)} | เลิก {int(r.closed_count)}")
                st.markdown("</div>", unsafe_allow_html=True)
            st.markdown("<div class='glass-card'><div class='section-eyebrow'>Micro-Niche Summary</div>เสาสูงและสีเขียวเข้ม = จังหวัดที่มีคะแนน Micro Opportunity สูงกว่า ส่วนสีเหลือง/ส้ม/แดง = จังหวัดที่ควรตรวจสอบความเสี่ยงเพิ่ม โดยดูร่วมกับจำนวนจัดตั้งและจำนวนเลิกกิจการ</div>", unsafe_allow_html=True)
